...
describe('ToolContextStore', () => {
  ...
  it('should get tool context by name', () => {
    ...
    expect(store.getToolContext('get_weather')).toEqual({
      apiKey: 'sk-123',
    });
  ...
  it('should set tool context and notify listeners', () => {
    ...
    store.setToolContext('get_weather', {
      apiKey: 'new',
    });
  ...
  it('should merge partial values into tool context and notify listeners', () => {
    ...
    store.subscribe((snapshot) => snapshots.push(snapshot));
    store.mergeToolContext('my_tool', {
      b: 3,
      c: 4,
    });

    expect(store.getToolContext('my_tool')).toEqual({
      a: 1,
      b: 3,
      c: 4,
    });
...
describe('buildToolExecuteContext', () => {
  ...
  it('should notify store listeners when setContext is called', () => {
    ...
    expect(snapshots).toHaveLength(1);
    expect(snapshots[0]?.get_weather).toEqual({
      apiKey: 'new',
    });
  });
});
...
describe('resolveContext', () => {
  ...
  it('should resolve sync function', async () => {
    const input = (turn: TurnContext) => ({
      my_tool: {
        turnNum: turn.numberOfTurns,
      },
    });
    const result = await resolveContext(input, turnContext);
    expect(result).toEqual({
  ...
  it('should resolve async function', async () => {
    const input = async (turn: TurnContext) => ({
      my_tool: {
        turnNum: turn.numberOfTurns + 1,
      },
    });
    const result = await resolveContext(input, turnContext);
    expect(result).toEqual({
...
describe('context mutation persistence across tool calls', () => {
  ...
  it('should persist mutations across calls via shared store', () => {
    ...
    // First tool call
    const ctx1 = buildToolExecuteContext<
      'my_tool',
      {
        counter: number;
      }
    ...
    const ctx2 = buildToolExecuteContext<
      'my_tool',
      {
        counter: number;
      }
    ...
    const ctx3 = buildToolExecuteContext<
      'my_tool',
      {
        counter: number;
      }
...
describe('tool isolation — each tool sees only its own context via local', () => {
  ...
  it('should expose own tool context via local', () => {
    ...
    const dbSchema = z4.object({
      connectionString: z4.string(),
    });

    const weatherCtx = buildToolExecuteContext<
      'get_weather',
      {
        apiKey: string;
      }
    >(turnContext, store, 'get_weather', weatherSchema);
    const dbCtx = buildToolExecuteContext<
      'db_query',
      {
  ...
  it('should not leak mutations between tools', () => {
    ...
    const ctxA = buildToolExecuteContext<
      'tool_a',
      {
        count: number;
      }
    >(turnContext, store, 'tool_a', schema);
    const ctxB = buildToolExecuteContext<
      'tool_b',
      {
        count: number;
      }
...
describe('shared context via sharedSchema', () => {
  ...
  it('should allow setSharedContext to mutate shared store', () => {
    ...
    expect(store.getToolContext('shared')).toEqual({
      _sessionId: 'new',
    });
  });
  ...
  it('should share context across different tools', () => {
    ...
    const schemaA = z4.object({
      key_a: z4.string(),
    });
    const schemaB = z4.object({
      key_b: z4.string(),
    });
    const sharedSchema = z4.object({
      sessionId: z4.string(),
