...
describe('Multi-Turn Tool State E2E Tests', () => {
  ...
  const createListTasksTool = () =>
    tool({
      ...
      execute: async () => ({
        tasks: Array.from(taskStore.values()).map((t) => ({
          id: t.id,
          name: t.name,
        })),
      }),
  ...
  const createDeleteTaskTool = () =>
    tool({
      ...
      execute: async (params) => {
        const entry = Array.from(taskStore.entries()).find(([_, t]) => t.name === params.name);
        if (entry) {
    ...
    for (const expected of expectedItems) {
      if (checkAllRequests) {
        ...
        for (const request of requestsToCheck) {
          ...
          expect(found, `Expected item ${JSON.stringify(expected)} missing from request`).toBe(
            true,
          );
        }
      } else {
        ...
        expect(
          foundInAny,
          `Expected item ${JSON.stringify(expected)} not found in any request`,
        ).toBe(true);
      }
    }
  }
  ...
  function verifyRequestStructure(request: CapturedRequest): void {
    ...
    if (firstNonSystemIdx >= 0 && systemIndices.length > 0) {
      ...
      for (const idx of systemIndices) {
        expect(idx, 'System messages must appear before non-system messages').toBeLessThan(
          firstNonSystemIdx,
        );
      }
    }
  ...
  function getItemKey(item: StreamableOutputItem): string | undefined {
    ...
    const id = 'callId' in item && item.callId ? item.callId : 'id' in item ? item.id : undefined;
  ...
  describe('User-Managed Multi-Turn Conversation', () => {
    it('should maintain cumulative item map across multiple user-managed conversation turns', async () => {
      ...
      const itemTypes = new Set(
        [
          ...allItemsMap.values(),
        ].map((item) => ('type' in item ? item.type : undefined)),
      );
      ...
      if (functionCallKeys.length > 0) {
        ...
        for (const callKey of functionCallKeys) {
          ...
          expect(allItemsMap.has(outputKey), `Missing function_call_output for ${callId}`).toBe(
            true,
          );
        }
      }
    ...
    it('should emit all item types from getItemsStream cumulatively', async () => {
      ...
      // Check item types
      const types = new Set(itemsReceived.map((item) => ('type' in item ? item.type : undefined)));
    ...
    it('should execute multiple tools in single callModel with pre-existing tasks', async () => {
      ...
      taskStore.set('task_1', {
        id: 'task_1',
        name: 'Alpha Task',
        createdAt: new Date().toISOString(),
      });
      taskStore.set('task_2', {
        id: 'task_2',
        name: 'Beta Task',
        createdAt: new Date().toISOString(),
      });
  ...
  describe('Multi-Turn Conversation with Async Input Function', () => {
    it('should maintain cumulative item map with input as function', async () => {
      ...
      const systemPrompt =
        'You are a task management assistant. Use the provided tools to manage tasks.';
      ...
      const inputFn = async (ctx: TurnContext): Promise<OpenResponsesInputUnion> => {
        ...
        const prevMessages: OpenResponsesInputUnion1[] =
          typeof prevInput === 'string' ? [] : (prevInput ?? []);
  ...
  describe('User Input Preservation', () => {
    it('should preserve original user input in ALL API requests during internal multi-turn tool execution', async () => {
      ...
      const originalUserMessage =
        'First create task "Alpha", then list all tasks using list_tasks tool';
    ...
    it('should preserve all user messages across multiple callModel calls', async () => {
      ...
      conversationInput.push({
        role: 'user',
        content: 'Create a task named "First Task"',
      });
      ...
      conversationInput.push({
        role: 'user',
        content: 'Now create another task named "Second Task"',
      });
      ...
      verifyInputItemsPreserved(
        ...
        {
          fromRequestIndex: call1RequestCount,
          checkAllRequests: true,
        },
      );
